	...
	function library()
		...
		library.theme = {
			...
			Selected = Color3.fromRGB(85, 0, 25);
	...
	local Window = DeleteMobLib:CreateWindow(HyperEscape.GUI.GUIToggleKey, "Freaky");
	...
	EnableSector:CreateToggle("Highlights", HyperEscape.esp.Highlights.Highlights, function(EHi) HyperEscape.esp.Highlights.Highlights = EHi; end);
	...
	ESPSettingsSecor:CreateToggle("Team Check", HyperEscape.esp.Highlights.TeamCheck, function(ETC) HyperEscape.esp.TeamCheck = ETC; end);
	EnableSector:CreateToggle("Show Highlight Through Walls", HyperEscape.esp.Highlights.AllWaysVisible, function(ESHTW) HyperEscape.esp.Highlights.AllWaysVisible = ESHTW; end);
	if not IsArsenal then
		ESPSettingsSecor:CreateColorPicker("Highlights Outline", HyperEscape.esp.Highlights.OutlineColor, function(EOCC) HyperEscape.esp.Highlights.OutlineColor = EOCC; end);
		ESPSettingsSecor:CreateSlider("Highlight Outline", 0, HyperEscape.esp.Highlights.OutlineTransparency * 100, 100, 1, function(EHOT) HyperEscape.esp.Highlights.OutlineTransparency = EHOT / 100; end);
		ESPSettingsSecor:CreateSlider("Highlight Fill", 0, HyperEscape.esp.Highlights.FillTransparency * 100, 100, 1, function(EHFT) HyperEscape.esp.Highlights.FillTransparency = EHFT / 100; end);
	end

	local SettingsTab = Window:CreateTab("Settings");
	local SettingsInfoSector = SettingsTab:CreateSector("Info", "Left");
	SettingsInfoSector:CreateCoppyText("Made By lite");
	...
	local function LoadESP(Player) -- I Thought It Would Be Harder To Make, Idk Why
		...
		local HighlightHolder = Instance.new("Folder", PlayerESP);
		HighlightHolder.Name = "Highlight"
		...
		local Highlight = Instance.new("Highlight", HighlightHolder);
		Highlight.Enabled = false;

		local co = coroutine.create(function()
			game:GetService("RunService").RenderStepped:Connect(function()
				if IsAlive(Player) and (HyperEscape.esp.Box.Box or HyperEscape.esp.Box.HealthBar or HyperEscape.esp.Box.Name or HyperEscape.esp.Box.Health or HyperEscape.esp.Tracer.Tracer or HyperEscape.esp.Highlights.Highlights) then
					...
					if onScreen and (HyperEscape.esp.TeamCheck ~= true  or GetTeam(Player) ~= GetTeam(localPlayer)) and HyperEscape.esp.Enabled then	
						...
						if HyperEscape.esp.MaxDistance > DistanceNumb then
							...
							if HyperEscape.esp.Highlights.Highlights then

								Highlight.Enabled = HyperEscape.esp.Highlights.Highlights;

								if not IsArsenal then
									Highlight.Adornee = Player.Character;
								end

								Highlight.OutlineColor = HyperEscape.esp.Highlights.OutlineColor;
								Highlight.FillColor = HyperEscape.esp.Highlights.FillColor;

								Highlight.FillTransparency = HyperEscape.esp.Highlights.FillTransparency;
								Highlight.OutlineTransparency = HyperEscape.esp.Highlights.OutlineTransparency;

								Highlight.DepthMode = (HyperEscape.esp.Highlights.AllWaysVisible and "AlwaysOnTop" or not HyperEscape.esp.Highlights.AllWaysVisible and "Occluded");
							else
								Highlight.Enabled = false;
